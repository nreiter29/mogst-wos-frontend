...
const Filter: React.FC<{ setFacetId: (id: null | number[]) => void, facetId: null | number[] }> = ({ setFacetId, facetId }) => {
                    ...
                    {data?.data.facets.items.map((facet, index) => {
                        return (
                            <Box key={"Filter" + facet && index} borderBottom="1px" borderColor="blackAlpha.400">
                                <FilterFolded facet={facet} setFacetId={setFacetId} refetch={refetch} facetId={facetId} selectedFacets={selectedFacets} setSelectedFacets={setSelectedFacets} />
                            </Box>
                        )
                    })
                    }
